...
(() => {
  ...
  function programs(){
    ...
    $('program-list').innerHTML=rows.slice(0,state.programLimit).map(p=>{const slug=M.fold(p.name).includes('inmunoprevenibles')?'inmunizaciones':M.fold(p.name)==='desarrollo de la educacion superior'?'educacion-superior':null;return `<details class="program" data-program-id="${esc(p.id)}" ${openPrograms.has(p.id)?'open':''}><summary><span class="program-name">${esc(p.name)}<small>${esc(p.entity)} · ${esc(p.jurisdiction)}${p.project_code?` · Programa ${p.project_code}`:''}</small></span><span class="program-amount">${short(val(p.project))}<small>Ver detalle +</small></span></summary><div class="program-detail"><p>El proyecto asigna <strong>${money(val(p.project))}</strong>. ${p.matched?'Las variaciones muestran tanto el cambio en pesos como su poder de compra.':'Falta verificar una correspondencia única con 2026; no se clasifica como un programa nuevo.'}</p>${programEvidence(p)}${comparisons(p)}<dl><dt>Inicial 2026</dt><dd>${short(val(p.law,2026))}</dd><dt>Vigente 2026 · 15/09</dt><dd>${short(val(p.current,2026))}</dd><dt>Proyecto 2027</dt><dd>${short(val(p.project))}</dd><dt>Ejecución 2026 · nominal</dt><dd>${M.ratio(p.accrued,p.current)===null?'—':nf(M.ratio(p.accrued,p.current))+'%'}</dd></dl><p class="note">${unit()}. — = sin comparación publicada. ${origin(p.source,p.page)}</p><div class="decision-links">${slug?`<a href="#politica-${slug}">Ver presupuesto, pagos y prestaciones →</a>`:''}<button class="button button-quiet" data-copy-program="${esc(p.id)}">Copiar enlace</button></div><p class="copy-status" role="status"></p></div></details>`;}).join('')||'<p class="empty">No encontramos coincidencias. Probá con otra palabra.</p>';
