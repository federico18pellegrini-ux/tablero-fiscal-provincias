...
(() => {
  ...
  const source=(name,page)=>{const s=[...D.sources,...(D.program_join?.sources||[])].find(s=>s.file===name);return s?`${s.url}${page?'#page='+page:''}`:'https://www.mecon.gob.ar/onp/presupuestos/2027';};
  ...
  function works(){
    const q=fold($('work-search').value),packageWorks=window.nationalDecisionContext?.works,byId=new Map(packageWorks?.rows.map(w=>[w.id,w.funding])||[]);$('work-funding').disabled=!packageWorks;const filter=$('work-funding').value;const rows=byProject(D.works.filter(w=>(!state.province||w.province===state.province)&&fold([w.name,w.entity,w.jurisdiction,w.province].join(' ')).includes(q)&&(filter==='all'||window.NationalDecisionMath.fundingMatches(byId.get(w.id),filter))));
    ...
    $('works-list').innerHTML=rows.length?rows.map(w=>`<article class="work"><small>${esc(w.province)} · ${esc(w.entity)}</small><h3>${esc(w.name)}</h3><span class="amount">${money(val(w.project))}</span>${byId.has(w.id)?`<p class="work-funding">${Object.entries(byId.get(w.id)).filter(([k,v])=>v>0&&!['total','internas','externas'].includes(k)).map(([k,v])=>`${esc(packageWorks.columns[k])}: ${money(val(v))}`).join('<br>')}</p>`:''}<small>${origin(w.source,w.page,'Ver partida ↗')}${w.id===packageWorks?.pilot?.project?.id?' · <a href="#obra-ra10">Ver ficha completa →</a>':''}</small></article>`).join(''):'<p class="empty">No hay partidas que coincidan con esta búsqueda.</p>';
  ...
  function reviewSources(r){
    return [...r.evidence.map(e=>origin(e.source,e.pages[0],`${e.source.startsWith('D26')?'2026':'2027'} · p. ${e.pages[0]} ↗`)),...(D.program_join.legal_evidence||[]).filter(e=>e.ids.includes(r.id)).map(e=>`<a href="${esc(e.url)}" target="_blank" rel="noopener">${esc(e.title)} ↗</a>`)].join(' · ');
  }
  function groupComparison(g){
    return `<div class="program-group" data-program-group="${esc(g.id)}"><h4>${esc(g.name)}</h4><p>${esc(g.note)}</p><dl><dt>Vigente 2026 · 15/09</dt><dd>${short(val(g.current,2026))}</dd><dt>Proyecto 2027</dt><dd>${short(val(g.project))}</dd></dl>${comparisons(g)}<p class="note">${unit()}. Comparación del conjunto: incluye esta partida y no se suma a ella.</p></div>`;
  }
  function programs(){
  ...
  function changes(){
    ...
    const list=(label,rows)=>`<div><h3>${label}</h3>${rows.length?rows.map(r=>`<article class="change-row"><strong class="${sign(diff(r))}">${diff(r)>0?'+':''}${short(diff(r))}</strong><p>${esc(r.name)}</p>${r.comparison_context?'<p class="note">El 99,3% del proyecto corresponde a actos electorales.</p>':''}<small>${pct(ch(r))} respecto de ${esc(baseLabel().toLowerCase())}</small></article>`).join(''):'<p class="empty">No hay variaciones en este sentido entre las partidas comparables.</p>'}</div>`;
  ...
  function methodology(){
    ...
    $('coverage-method').textContent=`Se extrajeron ${D.programs.length} partidas programáticas y ${D.works.length} partidas de proyectos. ${D.meta.program_join_matched} tienen correspondencia con 2026: ${D.meta.program_join_matched-D.program_join.documented_count} por nombre, entidad y jurisdicción, y ${D.program_join.documented_count} por códigos y documentos oficiales. Las ${D.programs.length-D.meta.program_join_matched} restantes tienen cambios de alcance identificados. Los 20 casos que estaban pendientes fueron revisados: ${D.program_join.new_links_count} se resolvieron individualmente. Hay además ${D.program_join.groups.length} comparaciones de conjuntos.`;
    $('program-reviews').innerHTML=[...D.program_join.reviews,...D.program_join.related_reviews].map(r=>`<article class="correspondence-review"><h4>${esc(r.name)}</h4><p><strong>${r.status==='comparable'?'Comparación incorporada':'Alcance reorganizado'}.</strong> ${esc(r.reason)}</p>${r.needed?`<p class="note">Falta para comparar la partida por separado: ${esc(r.needed)}</p>`:''}<p class="note">${reviewSources(r)} · <a href="?programa=${esc(r.id)}#programas">Ver partida →</a></p></article>`).join('');
  ...
  function download(){
    const header=['grupo','id','nombre','etapa_2027','unidad','proyecto_2027','inicial_2026','vigente_2026_2026-09-15','cierre_estimado_2026','devengado_2026_nominal','fuente','codigo_programa_2027','clave_2026','correspondencia','fuente_correspondencia'];
    const rows=[header];
    for(const group of ['jurisdictions','functions','geographies','programs','works','resources'])for(const r of D[group])rows.push([group,r.id||'',r.name,'Proyecto de ley','ARS millones · '+unit(),val(r.project),val(r.law,2026),val(r.current,2026),val(r.closing,2026),r.accrued,source(r.source,r.page),r.project_code??null,r.current_key?.join('/')||(r.current_parts?JSON.stringify(r.current_parts):null),r.match_note||r.review?.reason,r.match_source?`${r.match_source.url}#page=${r.match_source.page}`:null]);
    for(const g of D.program_join.groups)rows.push(['conjunto_no_sumar',g.id,g.name,'Proyecto de ley','ARS millones · '+unit(),val(g.project),val(g.law,2026),val(g.current,2026),null,g.accrued,'https://tablero.federicopellegrini.com.ar/nacion/data/program-sources/crosswalk.json',null,JSON.stringify(g.current_parts),g.note+' '+g.scope,null]);
  ...
  $('province').addEventListener('change',()=>selectProvince($('province').value));$('work-search').addEventListener('input',works);$('work-funding').addEventListener('change',works);window.addEventListener('national:decisions-ready',()=>{if(D)works();});
