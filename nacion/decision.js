...
(() => {
  ...
  function route(){
    ...
    for(const el of document.querySelectorAll('.policy-page'))el.hidden=el.id!=='politica-'+slug;
  }
  function finance(){
  ...
  function physicalCard(r){
    ...
    return `<article class="policy-measure"><h4>${esc(r.medicion_fisica_desc)}</h4>${r.subprograma_id?`<p class="note">${esc(r.subprograma_desc)}</p>`:''}<p class="note">${esc(r.unidad_medida_desc)} · ${r.totalizador_avance_fisico.startsWith('Promedio')?'Promedio del semestre':r.totalizador_avance_fisico.startsWith('Suma')?'Acumulado del semestre':'Indicador de corte'}</p><dl><div><dt>Previsto a junio</dt><dd>${num(v.planned,1)}</dd></div><div><dt>Realizado a junio</dt><dd>${num(v.actual,1)}</dd></div></dl>${v.partial?'<p class="information-status">Información parcial</p>':''}${v.change!==null?`<p class="note">Desvío frente a lo programado: <strong class="${v.change<0?'negative':''}">${pct(v.change)}</strong>.</p>`:'<p class="note">Falta información para comparar lo previsto y lo realizado.</p>'}${v.comments.length?`<p class="official-cause"><strong>Explicación del organismo.</strong> ${v.comments.map(esc).join(' ')}</p>`:''}</article>`;
  }
  function policy(p){
  ...
  function showError(message){
    for(const id of ['financiamiento',...Array.from(document.querySelectorAll('.policy-page'),el=>el.id),'carteras','obra-ra10','escenarios','attention'])$(id).innerHTML=`<p role="status">${esc(message)} <a href="">Reintentar</a>.</p>`;
    route();
  }
