(function(root){
  ...
  function selectReport(manifest, options, dataHash, managementHash,decisionHash){
    const {price,base,annex=false,scope='general'}=options;
    ...
    if(!['general','inmunizaciones','educacion-superior','reactor-ra10'].includes(scope))throw Error('Seleccioná un contenido válido.');
    const entry=manifest.reports?.find(r=>r.price===price&&r.base===base),pdf=scope==='general'?entry?.[annex?'full':'main']:manifest.focused?.find(r=>r.scope===scope);
    const expected=scope==='general'?`informe-nacional-${price}-${base}${annex?'-anexo':''}.pdf`:`ficha-nacional-${scope}.pdf`;
