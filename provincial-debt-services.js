...
function renderProvinceServices(province){
 ...
 if(!provincialServicesData){
  ...
  if(!provincialServicesRequest)provincialServicesRequest=fetch('data/provincial_debt_services.json?v=20260915-9').then(r=>{if(!r.ok)throw Error('debt services');return r.json();}).then(d=>{provincialServicesData=d;renderProvinceServices(currentProvince);renderProfileReadings();}).catch(()=>{provincialServicesRequest=null;document.getElementById('debtScheduleReading').textContent='No se pudo cargar la fuente. Cambiá de provincia para reintentar.';});
  return;
 }
 ...
 document.getElementById('debtScheduleHighlightLabel').textContent=projection?'servicios previstos · '+(projection.rows[0].period_label||'2026'):'calendario futuro';
 ...
 const method=document.createElement('p');method.className='federal-note';method.textContent='Obligaciones registradas en cada año, en pesos corrientes. No son vencimientos futuros ni pagos de caja.';history.append(method);
