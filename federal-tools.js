...
function initFederalTools(){
  ...
  <details id="fiscalHistoryPanel"><summary>Historia fiscal · <span id="historyProvince"></span></summary><div class="federal-content"><p>Cada punto suma los <strong>últimos 12 meses</strong> hasta esa fecha.</p><div class="history-chart"><canvas id="fiscalHistoryChart" aria-label="Evolución del resultado primario y financiero" role="img"></canvas></div><p id="historyReading" aria-live="polite"></p><details class="history-values"><summary>Ver los valores de cada período</summary><div class="federal-table"><table><caption>Resultados por cada $100 de ingresos · última versión publicada en cada informe</caption><thead><tr><th>Cierre</th><th>Primario</th><th>Financiero</th><th>Cambio financiero</th></tr></thead><tbody id="historyRows"></tbody></table></div></details><p class="federal-note">Los cambios pueden incluir revisiones de los datos publicados. Un espacio sin dato no representa cero.</p><a href="data/fiscal_history.json" download>Descargar datos y referencias</a></div></details>
...
async function renderNationalPanel(){
  ...
  try{
    ...
    const source=(parent,url,date,label='Fuente oficial')=>{const p=document.createElement('p');p.className='federal-note'+(label.startsWith('Descargar')?'':' source-reference');const a=document.createElement('a');a.href=url;a.target='_blank';a.rel='noopener';a.textContent=date?`${label} · publicación: ${date}`:label;p.append(a);parent.append(p);};
    ...
    paragraph(host,`Lectura para la gestión · datos revisados al ${d.reviewed_at}. Cada bloque tiene su propio período y cobertura. Las cifras son observadas, salvo el cálculo salarial identificado como estimación.`);
...
function renderComposition(province){
  ...
  const source=document.getElementById('compositionSource');source.textContent=debt?'Hasta 2T24 el stock se publica en ARS; desde 3T24, en USD equivalentes. No se comparan ni se suman montos de distinta moneda. Los huecos conservan datos faltantes. ':'Cociente de cada componente sobre ingresos totales, incluida seguridad social. No es una variación real del gasto ni una apertura por función (salud, educación o seguridad). ';
