...
test('all jurisdictions render dates, missingness and no collapsed content',()=>{
  for(const name of Object.keys(data.provinces)){
    ...
    assert.match(html,/reclamos, acuerdos y cobros conservan el alcance/);
    assert.equal((html.match(/data-claim-province=/g)||[]).length,0); // Province selection stays in the fixed menu.
