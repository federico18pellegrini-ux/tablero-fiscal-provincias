...
function renderPbaComparison(){
 ...
 if(typeof Chart!=='undefined')pbaSpendingChart=new Chart(canvas,{type:'bar',data:{labels:components.map(r=>r.key==='municipal_current_transfers'?['Transferencias','a municipios']:r.key==='other_transfers'?['Otras transferencias','corrientes']:r.label),datasets:[{data:components.map(r=>r.real_change_pct),backgroundColor:components.map(r=>r.real_change_pct<0?'#b84242':'#14796f'),borderRadius:4}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,animation:{duration:350},plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>fnum(c.raw)+'% real'}}},scales:{x:{ticks:{callback:v=>v+'%'}},y:{ticks:{font:{size:13}}}}}});
