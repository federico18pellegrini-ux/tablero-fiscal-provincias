...
def build(source):
    ...
    for raw in csv.DictReader(source):
        ...
        p=raw['period']
        v.update(province=ALIASES.get(raw['province_source'],raw['province_source']),period=f'20{p[-2:]}-Q{p[0]}',publication_date=raw['publication_date'],source_title=f'Informe fiscal provincias {p}',source_page=8,primary_pct=100*v['primary']/v['income'],financial_pct=100*v['financial']/v['income'])
        rows.append(v)
    keys=[(r['province'],r['period']) for r in rows]
    if len(keys)!=len(set(keys)):raise ValueError('Duplicate province-period')
    return {'schema_version':1,'unit':'ARS millions','frequency':'rolling_12_months','vintage_policy':'Valores publicados en cada informe; las revisiones pueden afectar las variaciones.','source':'Compilación de ejecuciones fiscales provinciales.','rows':sorted(rows,key=lambda r:(r['period'],r['province']))}
