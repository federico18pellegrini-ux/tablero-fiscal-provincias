...
function renderOverview(){
  ...
  $('insights').innerHTML=[
    ...
    ['03',fiscal?'El resultado de las cuentas':'La cuenta fiscal sigue abierta',fiscal?`Al 30 de junio de 2026, el resultado financiero fue ${pct(fiscal.resultado_sobre_ingresos_pct,2)} de los ingresos. Un resultado positivo no equivale a caja disponible: también importan los pagos pendientes y los fondos afectados.`:'Todavía falta una ejecución fiscal individual homologada para este municipio. Con las transferencias solas no podemos calcular su déficit, la inversión total ni cuánto dinero tiene disponible.','Ver las cuentas','recursos']
  ].map(([n,t,b,l,v])=>`<article class="insight"><span class="number">${n} /</span><h3>${t}</h3><p>${b}</p><button class="text-button" data-insight-view="${v}">${l} →</button></article>`).join('');
  $('insights').querySelectorAll('button').forEach(b=>b.onclick=()=>navigate(b.dataset.insightView));
...
function renderRanking(){
  ...
  $('ranking-coverage').textContent=`${ranked.length} ${ranked.length===1?'municipio':'municipios'} con dato comparable${meta.group==='Cuentas'?' · Muestra parcial, no ranking de los 135':''}${scopePeers?` · Entre ${num(m.poblacion_2022/2)} y ${num(m.poblacion_2022*2)} habitantes (Censo 2022)`:''}`;
...
function renderResources(){
  ...
  if(m.fiscal){const f=m.fiscal;$('fiscal-panel').innerHTML=`<span class="coverage-tag">Cuenta individual verificada · muestra parcial</span><h3>Las cuentas de ${escape(m.municipio)}</h3><p>Acumulado al 30 de junio de 2026. Recursos percibidos y gastos devengados, en pesos corrientes. El resultado financiero no mide caja disponible.</p><div class="stats-grid">${card('Resultado financiero',millions(f.resultado_financiero),'Millones de pesos corrientes',tone(f.resultado_financiero))}${card('Resultado sobre ingresos',pct(f.resultado_sobre_ingresos_pct,2),'Negativo: déficit · positivo: superávit',tone(f.resultado_sobre_ingresos_pct))}${card('Gasto de capital',num(f.capital_sobre_gasto_pct,1)+'%','Del gasto total, sin aplicaciones financieras')}</div><p class="chart-caption">La cobertura institucional y los servicios a cargo pueden diferir entre municipios. Fecha inicial declarada: ${f.inicio==='2026-01-01'?'1':'2'} de enero de 2026.</p>`;}
  else{$('fiscal-panel').innerHTML=`<span class="coverage-tag">Cobertura fiscal en ampliación</span><h3>Para cerrar las cuentas falta información</h3><p>Todavía no tenemos una ejecución fiscal individual homologada de ${escape(m.municipio)}. No podemos deducir el déficit o la inversión total a partir de las transferencias. Ya se pueden consultar estas tres cuentas al primer semestre de 2026:</p><div class="fiscal-choices">${rows.filter(r=>r.fiscal).map(r=>`<button data-municipality="${r.id}">${escape(r.municipio)} →</button>`).join('')}</div>`;bindMunicipalButtons($('fiscal-panel'));}
}
...
function attachEvents(){
  ...
  $('download-municipality').onclick=()=>{const m=current();download(`municipio-${m.id}.csv`,csv([['Municipio','Indicador','Valor','Unidad','Período','Criterio'],...METRICS.map(meta=>[m.municipio,meta.label,metricValue(m,meta),meta.unit,meta.period,meta.note])]));};
...
async function init(){
  try{
    const responses=await Promise.all([fetch('data/dashboard.json?v=20260907-1'),fetch('data/geografia_original.geojson')]);
