...
let reportTopics=[...DEFAULT_REPORT_TOPICS],reportGeneration=0,reportBusy=false,reportBlob=null,pdfLibraryPromise;
let deflator,priceMode='real',priceBase='2026-07',priceIndicator='transferencias';
...
function updateReportLink(m){
  const link=$('export-report');
  link.setAttribute('aria-label',`Elegir el contenido del informe de ${m.municipio}`);link.setAttribute('aria-disabled','false');
  $('export-report-label').textContent='Armar informe';
  $('report-scope').textContent=`${m.municipio} · Versión breve de 3 páginas o temas a elección.`;
  link.href=`?municipio=${m.id}&vista=informe`;link.removeAttribute('download');link.removeAttribute('target');
}
async function prepareReports(dashboardText){
  try{
    ...
    if(manifest.version!==3||manifest.input_sha256?.['municipios/data/dashboard.json']!==hash||!Array.isArray(manifest.reports)||manifest.reports.length!==rows.length||new Set(manifest.reports.map(r=>r.id)).size!==rows.length||manifest.reports.some(r=>!byId.has(r.id)||!validReportEntry(r)))throw new Error('Los informes están en actualización.');
  ...
  if(state.view==='informe')renderReportBuilder();
}
function clearReportDownload(){reportGeneration++;reportBusy=false;if(reportBlob)URL.revokeObjectURL(reportBlob.url);reportBlob=null;$('report-download-status').textContent='';}
function renderReportBuilder(){
  const m=current(),entry=reportManifest?.reports.find(r=>r.id===m.id),link=$('download-report');
  $('report-title').textContent=`El informe de ${m.municipio}`;
  $('report-options').innerHTML='';link.removeAttribute('href');link.setAttribute('aria-disabled','true');
  if(!entry){$('report-selection-summary').textContent=reportUnavailable?'El informe está en actualización. Los datos del tablero siguen disponibles.':'Cargando las opciones del informe…';$('report-outline').innerHTML='';return;}
  const selected=reportSelection(entry,reportTopics),ids=new Set(selected.modules.map(x=>x.id));
  $('report-options').innerHTML=entry.modules.map(module=>`<label class="report-option ${ids.has(module.id)?'is-chosen':''}"><input type="checkbox" data-report-topic="${module.id}" ${ids.has(module.id)?'checked':''} ${module.required?'disabled':''}><span><strong>${escape(module.label)}</strong><span>${escape(module.description)}</span><small>${module.pages.length} ${module.pages.length===1?'página':'páginas'}${module.required?' · Siempre incluida':''}</small></span></label>`).join('');
  $('report-options').querySelectorAll('input').forEach(input=>input.onchange=()=>{const topic=input.dataset.reportTopic;clearReportDownload();const set=new Set(reportTopics);input.checked?set.add(topic):set.delete(topic);reportTopics=[...set];persist();renderReportBuilder();$('report-options').querySelector(`[data-report-topic="${topic}"]`)?.focus({preventScroll:true});});
  const missing=[['detalle-fiscal','Detalle de ejecución'],['caja','Caja y deuda municipal'],['historia','Historia fiscal'],['bancos','Saldos bancarios']].filter(([id])=>!entry.modules.some(x=>x.id===id)).map(([,label])=>label);
  $('report-unavailable-topics').textContent=missing.length?`Sin detalle verificable para incluir en este municipio: ${missing.join(', ')}. Estos faltantes no se convierten en cero.`:'';
  $('report-selection-summary').textContent=`${selected.count} ${selected.count===1?'página':'páginas'} · ${selected.mode==='brief'?'Informe breve recomendado':selected.mode==='full'?'Todos los temas disponibles':'Contenido elegido'}`;
  $('report-outline').innerHTML=selected.modules.map(module=>`<li>${escape(module.label)}</li>`).join('');
  $('report-preset-brief').setAttribute('aria-pressed',String(selected.mode==='brief'));$('report-preset-all').setAttribute('aria-pressed',String(selected.mode==='full'));
  const ready=selected.mode==='brief'?entry.brief:selected.mode==='full'?entry:null;
  const matchedBlob=reportBlob?.key===selected.key?reportBlob:null;
  link.href=ready?`reports/${ready.file}?v=${ready.sha256}`:matchedBlob?matchedBlob.url:'#informe';
  link.download=`informe-${m.municipio.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-${selected.count}-paginas.pdf`;
  link.setAttribute('aria-disabled',String(reportBusy));link.textContent=reportBusy?'Preparando el PDF…':`Descargar informe · ${selected.count} ${selected.count===1?'página':'páginas'} ↓`;
}
async function pdfLibrary(){
  if(window.PDFLib)return window.PDFLib;
  if(!pdfLibraryPromise)pdfLibraryPromise=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='vendor/pdf-lib.min.js?v=1.17.1';script.onload=()=>{if(window.PDFLib)resolve(window.PDFLib);else{script.remove();pdfLibraryPromise=null;reject(new Error('No se pudo cargar la herramienta de PDF.'));}};script.onerror=()=>{script.remove();pdfLibraryPromise=null;reject(new Error('No se pudo cargar la herramienta de PDF.'));};document.head.append(script);});
  return pdfLibraryPromise;
}
async function downloadSelectedReport(event){
  const entry=reportManifest?.reports.find(r=>r.id===state.id);
  if(!entry||reportBusy){event.preventDefault();return;}
  const topics=[...reportTopics],selected=reportSelection(entry,topics);
  if(selected.mode!=='custom'||reportBlob?.key===selected.key)return;
  event.preventDefault();const generation=++reportGeneration;reportBusy=true;renderReportBuilder();$('report-download-status').textContent='Preparando sólo los temas elegidos…';
  try{
    const response=await fetch(`reports/${entry.file}?v=${entry.sha256}`);if(!response.ok)throw new Error('No se pudo descargar el documento de respaldo.');
    const bytes=await response.arrayBuffer(),digest=await crypto.subtle.digest('SHA-256',bytes);
    const hash=Array.from(new Uint8Array(digest),b=>b.toString(16).padStart(2,'0')).join('');
    if(hash!==entry.sha256)throw new Error('El documento está en actualización. Volvé a cargar la página.');
    const pdf=await composeMunicipalPdf(bytes,entry,topics,await pdfLibrary());
    if(generation!==reportGeneration||entry.id!==state.id)return;
    reportBlob={key:selected.key,url:URL.createObjectURL(new Blob([pdf],{type:'application/pdf'}))};reportBusy=false;renderReportBuilder();
    $('report-download-status').textContent=`Informe preparado: ${selected.count} páginas. Si no se descargó automáticamente, usá el botón de descarga.`;
    $('download-report').click();
  }catch(error){if(generation===reportGeneration){reportBusy=false;renderReportBuilder();$('report-download-status').textContent=error.message+' Podés reintentar o elegir la versión breve.';}}
}
...
function selectMunicipality(id){if(!byId.has(id))return;clearReportDownload();state.id=id;persist();renderView();}
...
function renderOverview(){
  ...
  const priorities=[
    ['01','Cuidar los recursos para sostener los servicios',
      `${r<0?'Esos fondos alcanzan para menos y presionan sobre los servicios y las obras.':'Antes de convertir esa mejora en gastos permanentes, conviene verificar si se mantiene y qué fondos tienen un destino específico.'} La variación está ${num(Math.abs(diff),1)} puntos porcentuales ${diff>=0?'por encima':'por debajo'} del conjunto bonaerense.`,
    ...
    ['03','Entender qué está pasando con el trabajo',
      `${j<0?'La caída puede afectar a las familias y a los comercios que dependen de esos ingresos.':'El crecimiento abre oportunidades, aunque puede concentrarse en pocos sectores o empresas.'} Son puestos en establecimientos del municipio; no mide desocupación ni describe el empleo de 2026.`,
...
function renderRanking(){
  ...
  const reading=rankingReading(meta),referenceValue=metricValue(m,meta),referenceAssessment=rankingAssessment(meta,referenceValue);
  $('ranking-reading-text').textContent=reading.note;
  $('ranking-color-key').innerHTML=['negative','positive','neutral'].filter(tone=>reading[tone]).map(tone=>`<span class="ranking-key-item ${tone}"><i aria-hidden="true"></i>${escape(reading[tone])}</span>`).join('');
  ...
  $('ranking-selected').innerHTML=`<div class="rank-reference-info"><span class="rank-reference-label">Municipio de referencia</span><strong>${escape(m.municipio)}</strong><span>${selected?`Posición ${selected.rank} de ${ranked.length} en esta comparación`:meta.budget?'Sin presupuesto verificado para el tipo y corte elegidos':'Sin dato para este indicador'}</span>${selected&&meta.budget?`<span>${escape(rankingRowContext(m,meta))}</span>`:''}</div><div class="rank-reference-detail"><strong class="${referenceAssessment.tone}">${formatMetric(referenceValue,meta)}</strong><span class="rank-assessment-label ${referenceAssessment.tone}">${escape(referenceAssessment.label)}</span><button class="text-button" id="open-reference">${transparency?'Ver detalle de transparencia':meta.budget?'Ver presupuesto y documento':'Ver ficha municipal'} →</button></div>`;
  ...
  $('ranking-rows').innerHTML=(showAll?ranked:ranked.slice(0,10)).map(r=>{
    const pos=(r.value-lo)/span*100,left=Math.min(pos,zero),width=r.value===0?0:Math.max(Math.abs(pos-zero),.4),context=rankingRowContext(r.m,meta),assessment=rankingAssessment(meta,r.value);
    return `<button class="rank-row ${r.m.id===m.id?'is-selected':''}" data-municipality="${r.m.id}" aria-label="${escape(r.m.municipio)}, puesto ${r.rank}, ${formatMetric(r.value,meta)}. ${escape(assessment.label)}.${context?' '+escape(context)+'.':''} Seleccionar municipio"><span class="rank-number">${r.rank.toString().padStart(2,'0')}</span><span class="rank-name">${escape(r.m.municipio)}${context?`<small class="rank-row-context">${escape(context)}</small>`:''}</span><span class="rank-track" aria-hidden="true"><span class="rank-zero" style="left:${zero}%"></span><span class="rank-fill ${assessment.tone}" style="left:${left}%;width:${width}%"></span></span><span class="rank-value ${assessment.tone}" title="${escape(assessment.label)}">${formatMetric(r.value,meta)}</span></button>`;
  }).join('');
...
function renderPrices(){
  ...
  const comparisons=priceComparisons(current(),priceMode,priceBase,deflator);
  $('price-indicators').innerHTML=comparisons.map(r=>`<button data-price-indicator="${r.id}" aria-pressed="${r.id===priceIndicator}">${escape(r.label)}</button>`).join('');
  $('price-indicators').querySelectorAll('button').forEach(button=>button.onclick=()=>{priceIndicator=button.dataset.priceIndicator;persist();renderPrices();});
  $('price-cards').innerHTML=comparisons.filter(r=>r.id===priceIndicator).map(r=>{
...
function priceExport(){
  ...
  return [['Municipio','Indicador','Valor anterior','Período anterior','Valor actual','Período actual','Unidad de importes','Variación %','Criterio'],...priceComparisons(current(),priceMode,priceBase,deflator).filter(r=>r.id===priceIndicator).map(r=>[current().municipio,r.label,r.previous,r.previousPeriod,r.current,r.currentPeriod,unit,r.change,r.method])];
}
...
function attachEvents(){
  ...
  $('export-report').onclick=event=>{event.preventDefault();navigate('informe');};
  $('download-report').onclick=downloadSelectedReport;
  $('report-preset-brief').onclick=()=>{clearReportDownload();reportTopics=[...DEFAULT_REPORT_TOPICS];persist();renderReportBuilder();};
  $('report-preset-all').onclick=()=>{clearReportDownload();reportTopics=[...REPORT_TOPICS];persist();renderReportBuilder();};
...
async function init(){
  try{
    ...
    if(params.has('temas'))reportTopics=['lectura',...params.get('temas').split(',').filter(x=>REPORT_TOPICS.includes(x)&&x!=='lectura')];
