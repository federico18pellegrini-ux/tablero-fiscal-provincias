...
function renderFiscal(m){
  const f=m.fiscal,e=m.fiscalExecution,count=data.fiscalCoverage.fiscal;
  if(f){
    ...
    $('fiscal-panel').innerHTML=`<span class="coverage-tag">Cobertura fiscal en ampliación</span><h3>Para cerrar las cuentas falta información</h3><p>Todavía no tenemos una ejecución fiscal individual homologada de ${escape(m.municipio)}. Eso no significa que el municipio no la publique. No podemos deducir el déficit o la inversión total a partir de las transferencias. Ya hay ${count} cuentas verificadas con cierre en junio de 2026:</p><div class="fiscal-choices">${rows.filter(r=>r.fiscal).map(r=>`<button data-municipality="${r.id}">${escape(r.municipio)} →</button>`).join('')}</div>`;
    bindMunicipalButtons($('fiscal-panel'));
  }
