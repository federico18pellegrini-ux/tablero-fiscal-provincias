...
## Ampliación desde el directorio municipal — 7 de septiembre de 2026

Se cruzaron los 135 enlaces con las cuentas existentes y se buscaron documentos en los 117 casos sin CAIF incorporada, incluido Tigre con ejecución parcial. Se recuperaron 1.175 PDF distintos por contenido desde los portales y archivos públicos enlazados. Los originales, extracciones y pruebas visuales permanecen en la carpeta de investigación `outputs/municipios-portales-20260907` del espacio de trabajo, fuera del sitio publicado.

La cobertura pasa de 18 a 90 municipios con cuentas fiscales: 72 tienen acumulados comparables al 30 de junio de 2026 y otros 18 tienen cuentas de períodos diferentes. Bragado conserva además agosto de 2026; junio sigue siendo el período del ranking. Hay 19 observaciones de otros períodos, incluyendo esa observación adicional de Bragado. Personal cubre 71 municipios del semestre: Ayacucho conserva el faltante del primer trimestre.

La carga agrega 54 cuentas del semestre y 19 de otros períodos, sustentadas en 77 PDF verificados. Se revisaron visualmente los cuadros relevantes, la entidad, el período, los signos y la columna devengado de personal. En documentos escaneados o con fuentes que impiden extraer texto, se transcribieron los valores del documento renderizado. El importador verifica las identidades con decimales antes de publicar.

Ayacucho, General Guido, General Viamonte y San Isidro se calculan sumando dos trimestres consecutivos sin superposición. Las partes se conservan y se controlan individualmente, además de conciliar con el semestre. Se excluyeron como cuentas municipales los documentos de hospitales y entes publicados por separado; se preserva el alcance institucional de la administración municipal. En particular, General Pueyrredon corresponde a la administración central y Azul no usa la Dirección de Vialidad Rural.

Las fichas muestran el último período incorporado con fechas explícitas, botones visibles cuando existe más de un corte y una aclaración para los períodos ajenos al ranking. La descarga municipal incluye los importes de todas las cuentas incorporadas y sus períodos. Los rankings siguen usando exclusivamente el semestre comparable. Los puntajes históricos de ASAP, las transferencias, el empleo, la población y las cuentas previamente incorporadas se conservaron.

El registro `municipios/data/fiscal_search.json` identifica los 135 casos y sus enlaces consultados; `revision_portales.csv` ofrece una versión descargable. Quedan 44 municipios sin cuenta incorporada: 34 sin una cuenta verificable en las páginas recorridas, cuatro con acceso incompleto y seis con documentos que todavía no permiten completar una cuenta homologada. Tigre continúa con ejecución presupuestaria parcial. Estos estados describen la búsqueda, no prueban falta de publicación.

Validación: 87 pruebas Python y 41 Node aprobadas; compilación, sintaxis y controles de trazabilidad sin errores. Los 24 informes provinciales conservan sus huellas. Navegador a 320, 390, 768 y 1.264 px, sin desbordes horizontales ni errores de aplicación. Se probaron Bragado (cambio junio/agosto), La Plata (marzo fuera del ranking), Ayacucho (personal faltante), Lincoln (cuenta pendiente), los 72 renglones del ranking y el filtro por población. Se verificó el CSV generado por la interfaz: 72 municipios en el ranking y ambos períodos en la descarga de Bragado. Son verificaciones en navegador con tamaños de pantalla simulados, no pruebas en dispositivos físicos.
...
## Auditoría independiente de datos — 7 de septiembre de 2026

El [informe detallado](auditoria_municipal_2026-09-07.md) distingue la consistencia de los importes, las limitaciones de cobertura y los controles de publicación. Se verificaron nuevamente 111 archivos originales, más la cartografía Georef: todos coinciden por contenido con los archivos conservados. Cuatro PDF requirieron recuperar el enlace desde sus portales públicos. Se realizaron 70.783 comprobaciones independientes sobre planillas originales y cálculos, 1.890 sobre componentes de ASAP y una revisión fiscal de 99 PDF y 763 importes (712 por extracción, 48 mediante imagen y tres totales reconstruidos). No se detectaron diferencias numéricas.

Las tres cuentas iniciales de Trenque Lauquen, General Rodríguez y Carmen de Areco se incorporan al registro fiscal verificado, con los mismos importes. Las 72 cuentas del semestre pasan ahora por los mismos controles de documentos y conciliación. La reconstrucción descarta cuentas anteriores que ya no estén en el registro y rechaza respaldo documental incompleto. El CSV de Tigre incorpora sus cinco magnitudes presupuestarias, separadas del resultado fiscal y con fechas y unidades.

En navegador se recorrieron 540 vistas individuales y 108 configuraciones de ranking a 390 y a 768 px: 20.262 comprobaciones por tamaño, sin diferencias ni desbordes. Se comprobaron además 162 contenidos de exportación mediante los botones de la interfaz (135 municipios y 27 rankings), con 7.737 verificaciones. En esta corrida la descarga al disco fue cancelada por el navegador de prueba tanto localmente como en producción: se valida el contenido generado, sin afirmar que se verificó la entrega del archivo. Los importes, signos, posiciones y faltantes publicados se mantienen.

Pasaron 90 pruebas Python, 41 JavaScript, compilación, sintaxis, trazabilidad TOP (150 registros), reclamos nacionales (31) y vigencia de los 24 informes provinciales. Los originales, scripts independientes, resultados y capturas quedan en `outputs/municipios-auditoria-20260907` del espacio de trabajo, fuera del sitio publicado.
