...
function initExecutiveRedesign(){
  ...
  const extras=document.createElement('details');extras.className='overview-extra';extras.innerHTML='<summary>Ver resultado primario y posición en el ranking</summary><div class="overview-extra-grid"></div>';
...
function renderExecutiveOverview(){
  ...
  const latest=rows.find(r=>r.period===last);document.getElementById('overviewTrendReading').textContent=latest?`${province}: ${fnum(latest.financial_pct)}% al ${periodName(last)}. Fuente: 1816.`:`${province}: sin observación en ${periodName(last)}. No se prolonga el último dato.`;
  document.getElementById('overviewPeersReading').textContent=`${peers.length} jurisdicciones · ${periodName(last)} · fuente: 1816. Un saldo mayor no mide por sí solo la calidad de gestión.`;
}
